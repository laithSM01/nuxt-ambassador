      ...
      <div class="bg-white rounded-lg shadow p-6 max-w-2xl">
        ...
        <form v-else @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <div>
            ...
            <input @change="handleImageChange" type="file" accept="image/*" class="w-full border border-gray-300 rounded px-4 py-2" />
          </div>
          
          <div v-if="submitError" class="text-red-600 mt-4">{{ submitError }}</div>
...
<script setup>
  ...
  const { loading, error, fetchProductById, updateProduct } = useProducts()
